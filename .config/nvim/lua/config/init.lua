...
function M.json.load()
    local path = vim.fn.stdpath("config") .. "/json"
...
function M.get_kind_filter(buf)
    ...
    return type(M.kind_filter) == "table" and type(M.kind_filter.default) ==
               "table" and M.kind_filter.default or nil
end
...
function M.load(name)
    ...
    if M.defaults[name] or name == "options" then _load("config." .. name) end
    _load("config." .. name)
